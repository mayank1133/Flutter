...
void _checkIfFavorite() async {
  ...
  if (favoriteUserIds != null) {
    ...
    isFavorite = ids.contains (widget.user['id'].toString());
    setState(() {});
  }
}
...
void _updateFavoriteStatus() async {
  SharedPreferences prefs = await SharedPreferences.getInstance();
  String? favoriteUserIds = prefs.getString('favorite_users');
  List<String> ids = favoriteUserIds != null ? favoriteUserIds.split(',') : [];

  if (isFavorite) {
  ids.add(widget.user['id'].toString());
  } else {
  ids.remove(widget.user['id'].toString());
  }

  await prefs.setString('favorite_users', ids.join(','));
  }
